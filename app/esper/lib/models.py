...
class QuerySetMixin(object):
    ...
    def values_with(self, *fields):
        return self.values(
            *([f.name for f in self.model._meta.get_fields()] + list(fields)))
...
class Video(models.Model):
    ...
    def _ffmpeg_fmt_time(self, t):
        return '{:02d}:{:02d}:{:02d}.{:03d}'.format(int(t / 3600),
                                                    int(t / 60 % 60),
                                                    int(t % 60),
                                                    int(t * 1000 % 1000))
    ...
    def download(self, output_path=None, ext='mp4', segment=None):
        if output_path is None:
            output_path = tempfile.NamedTemporaryFile(suffix='.{}'.format(ext),
                                                      delete=False).name
        ...
        sp.check_call('ffmpeg -y {} -i "{}" {} {}'.format(
            start_str, self.url(), end_str, output_path),
                      shell=True)
        return output_path
    ...
    class Track(models.Model):
        ...
        def duration_expr():
            return ExpressionWrapper(
                Cast(F('max_frame') - F('min_frame'), models.FloatField()) /
                F('video__fps'), models.FloatField())
...
class BoundingBox(models.Model):
    ...
    def height_expr():
        return ExpressionWrapper(
            F('bbox_y2') - F('bbox_y1'), models.FloatField())
...
class Features(models.Model):
    ...
    def compute_distances(cls, inst_id):
        ...
        it = cls.objects.annotate(height=F('face__bbox_y2') -
                                  F('face__bbox_y1')).filter(
        ...
        dists, indices = feat_nn.kneighbors(
            [cls.objects.get(face=inst_id).load_features()], 1000)
...
class Pose(models.Model):
    ...
    def face_keypoints(self):
        kp = self._format_keypoints()
        return kp[self.POSE_KEYPOINTS:(self.POSE_KEYPOINTS +
                                       self.FACE_KEYPOINTS), :]
